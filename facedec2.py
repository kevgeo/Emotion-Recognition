...
img = cv2.imread("fac/AF01ANS.JPG")
...
for (x,y,w,h) in faces:
    ...
    for (ex,ey,ew,eh) in eyes:
        ...
        imgf = eye_roi[:, 0:eye_roi.shape[1]/2]
        imgk = eye_roi[:, eye_roi.shape[1]/2:eye_roi.shape[1]]

        corners = cv2.cornerHarris(np.float32(imgf), 2, 5, 0.04)
        ...
        cv2.circle(eye_roi_color, (my1_cord, mx1_cord), 5, (0, 0, 255), -1)

        corners = cv2.cornerHarris(np.float32(imgk), 2, 5, 0.04)
        dst = cv2.dilate(corners, kernel, iterations = 2)
        mx2_cord = int(np.where(dst == dst.max())[0].mean())
        my2_cord = int(np.where(dst == dst.max())[1].mean())
        cv2.circle(eye_roi_color, (eye_roi.shape[1]/2+my2_cord, mx2_cord), 5, (0, 0, 255), -1)
